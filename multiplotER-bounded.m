...
%
% $Id: ft_multiplotER.m 10196 2015-02-11 09:15:07Z roboos $

revision = '$Id: ft_multiplotER.m 10196 2015-02-11 09:15:07Z roboos $';
...
for i=1:Ndata
  ...
  dat  = varargin{i}.(cfg.parameter);
  dat_sem = varargin{i}.powspctrmsem;
  ...
  zdim_sem = setdiff(1:ndims(dat_sem), [ydim xdim]);

  % and permute
  dat = permute(dat, [zdim(:)' ydim xdim]);
  dat_sem = permute(dat_sem, [zdim(:)' ydim xdim]);
  ...
  elseif haslabelcmb
    ...
    dat = dat(sellab, xidmin(i):xidmax(i));
    dat_sem = dat_sem(sellab, xidmin(i):xidmax(i));
  end
  ...
  datamatrix_sem{i} = dat_sem(seldat, :);
...
for m=1:length(layLabels)
  ...
  ft_plot_vector(xval, yval, 'ysem', ysem, 'width', width(m), 'height', height(m), 'hpos', layX(m), 'vpos', layY(m), 'hlim', [xmin xmax], 'vlim', [ymin ymax], 'color', color, 'style', cfg.linestyle{i}, 'linewidth', cfg.linewidth, 'axis', cfg.axes, 'highlight', mask, 'highlightstyle', cfg.maskstyle, 'label', label, 'box', cfg.box, 'fontsize', cfg.fontsize);
